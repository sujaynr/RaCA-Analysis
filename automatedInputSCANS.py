...
common_args_base = ['python3', script_path, '--trainValSplit', '0.1', '--encoderModel', 's', '--fullFit', '--epochs', '500', '--allScans']
        ...
        for bs in batch_sizes:
            for seed in seeds:
                for scanSplit in scanSplits:
                    for model in models:
                        # Adjust the log name based on the model
                        if model == "--decoderModel":
                            log_name_suffix = "annVAL"
                        else:
                            log_name_suffix = "ND"

                        log_name = f"scansFINALSWEEP_{log_name_suffix}_{scanSplit}_lr{lr}_hl{hl}_bs{bs}_seed{seed}"
                        ...
                        if model == "--decoderModel":
                            command.append("--decoderModel")
                        else:
                            command.append("--noDecoder")

                        # Run the subprocess with the constructed command
                        result = subprocess.run(command, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)

                        # Output information about the current run
                        print(f"Running with lr={lr}, hl={hl}, bs={bs}, seed={seed}, scanSplit={scanSplit}, model={model}, logName={log_name}")
                        ...
                        if result.returncode == 0:
                            print(f"Execution succeeded for lr={lr}, hl={hl}, bs={bs}, seed={seed}, scanSplit={scanSplit}, model={model}")
                        else:
                            print(f"Execution failed for lr={lr}, hl={hl}, bs={bs}, seed={seed}, scanSplit={scanSplit}, model={model}")
